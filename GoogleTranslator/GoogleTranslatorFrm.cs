        ...
        #region Button handlers
			...
			#region Translate the text
                try {
					...
					//this._editSourceText.Text = this._editSourceText.Text.Replace("！", "!");
					_editSourceText.Text = t.Translate(_editSourceText.Text, (string)this._comboFrom.SelectedItem, (string)this._comboTo.SelectedItem,referenceTextUrl.Text);
					if (t.Error == null) 
					{
                        //this._editTarget.Update();
			...
			#endregion
                if(File.Exists(_editTarget.Text))
				{
					File.WriteAllText(_editTarget.Text, _editSourceText.Text);
				}
				else
				{
					using (StreamWriter sw = File.CreateText(_editTarget.Text))
					{
						sw.Write(_editSourceText.Text);
					}	
				}
            }
			/// <summary>
